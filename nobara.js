...
app.get('/', function(req, res) {
  res.sendFile(path.join(__dirname, '/kugisaki_nobara/login/cover/Marin.html'));
});
...
function uptime() {
  ...
  logger(`Starting up..`, "KUGISAKI NOBARA");
  startBot(0);
  ...
  function startServer(port) {
    app.listen(port, () => {
      logger.loader(`Nobara is running on port: ${port}`);
      logger.loader(`Activating uptime for ${chalk.underline(`'${REPL_HOME}'`)}`, 'KUGISAKI NOBARA');
    });

    app.on('error', (error) => {
      logger(`An error occurred while starting the server: ${error}`, "KUGISAKI NOBARA");
    });
  }
...
// # Please note that sometimes this function is the reason the bot will auto-restart, even if your custom.js auto-restart is set to false. This is because the port switches automatically if it is unable to connect to the current port. ↓↓↓↓↓↓
  ...
  async function startBot(index) {
    try {
      ...
      if (!isAvailable) {
        logger(`Retrying...`, "KUGISAKI NOBARA");
      ...
      const child = spawn("node", ["--trace-warnings", "--async-stack-traces", "kugisaki_nobara.js"], {
        cwd: __dirname,
        stdio: "inherit",
        shell: true,
        env: {
      ...
      child.on("error", (error) => {
        logger(`An error occurred while starting the child process: ${error}`, "KUGISAKI NOBARA");
      });
    } catch (err) {
      logger(`Error while starting the bot: ${err}`, "KUGISAKI NOBARA");
    }
  }
