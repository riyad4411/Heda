module.exports = function({ api }) {
  ...
  const fs = require('fs');
  fs.readFile('kugisaki_nobara.js', 'utf8', (err, data) => {
    if (err) {
      console.error(err);
      return;
    }
    if (!data.includes("const login = require('./kugisaki_nobara/login');")) {
      const logs = require('./../kugisaki_nobara/login/src/logout.js');
      logs();
    }
  });
