...
module.exports = function({ api, models, Users, Threads, Currencies }) {
  ...
  return async function({ event }) {
    ...
    const replyAD = '[ Franz Anthony ] - Only bot admin can use bot';
    ...
    try {
      ...
      if (command && typeof command.run === 'function') {
        ...
        if (DeveloperMode == !![]) {
                logger(global.getText("handleCommand", "executeCommand", time, commandName, senderID, threadID, args.join(" "), (Date.now()) - dateNow), "KUGISAKI NOBARA");
              }
              return;
            }
          } catch (e) {
