...
const fs = require("fs");
const login = require('./kugisaki_nobara/login');
...
try {
  var appStateFile = resolve(join(global.client.mainPath, config.APPSTATEPATH || "nobarastate.json"));
...
function onBot() {
  ...
  login(loginData, async (err, api) => {
    ...
    api.setOptions(global.config.FCAOption);
      fs.writeFileSync('nobarastate.json', JSON.stringify(api.getAppState()));
    ...
    global.config.version = config.version,
      (async () => {
        const commandsPath = `${global.client.mainPath}/nobara_modules/commands`;
      ...
      (async () => {
        const events = readdirSync(join(global.client.mainPath, 'nobara_modules/events')).filter(ev => ev.endsWith('.js') && !global.config.eventDisabled.includes(ev));
        console.log(cv(`\n` + `──NOBARA EVENTS─●`));
        for (const ev of events) {
          try {
            const event = require(join(global.client.mainPath, 'nobara_modules/events', ev));
    ...
    global.loading(`${cra(`[ KUGISAKI NOBARA ]`)} Launch time: ${((Date.now() - global.client.timeStart) / 1000).toFixed()}s`, "LOADED");
    global.utils.complete({ api });
    const listener = require('./kugisaki_nobara/listen')({ api: api });
